...
int main()
{
  ...
  while (1) {
    ...
    if (pid == 0) { // child
      ...
      execlp("java", "java", "-jar", "vdmj-3.0.1.jar", "-vdmsl", "simple.vdmsl", "-i", NULL);
      //execlp("java", "java", "-jar", "vdmj-3.0.1.jar", "-vdmpp", "counter.vdmpp", "-i", NULL);
      //execlp("java", "java", "-jar", "vdmj-3.0.1.jar", "-vdmsl", "SAFERSL/", "-default", "TEST", "-i", NULL);  //remember to set default module to SAFER
      return 0;
    }
    else { // parent
